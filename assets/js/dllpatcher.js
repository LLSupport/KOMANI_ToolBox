...
DllPatcherContainer.prototype.createUI = function () {
    ...
    label.html("<strong>选择文件</strong> 或 拖至此处.");
    ...
    if (versions.length > 0) {
        $("<h4>支持的版本：</h4>").appendTo(container);
    }
    ...
    reader.onload = function (e) {
        ...
        for (var i = 0; i < self.patchers.length; i++) {
            ...
            if (patcher.validatePatches()) {
                ...
                self.successDiv.html(successStr + " 加载成功!");
            }
        }

        if (!found) {
            self.errorDiv.html("没有找到此DLL对应版本！");
        }
    };
